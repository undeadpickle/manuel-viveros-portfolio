...
Both languages use the same English route names with different locale prefixes:

| Page | English | Spanish |
|------|---------|---------|
| Home | `/en` | `/es` |
| Paintings | `/en/paintings` | `/es/paintings` |
| Sculptures | `/en/sculptures` | `/es/sculptures` |
| Sketches | `/en/sketches` | `/es/sketches` |
| Photography | `/en/photography` | `/es/photography` |
| Journals | `/en/journals` | `/es/journals` |
| Journal Entry | `/en/journals/[slug]` | `/es/journals/[slug]` |
| Contact | `/en/contact` | `/es/contact` |

### Planned: Localized Routes (Future Enhancement)
Route translations are defined in `src/lib/i18n.ts` for potential future implementation:

| Page | English | Spanish (planned) |
|------|---------|---------|
...
src/app/[lang]/paintings/page.tsx → /en/paintings or /es/paintings
```
...
// If on /en/paintings → returns { locale: 'es', path: '/es/paintings' }
// (Route translation is available but currently swaps locale prefix only)
```
